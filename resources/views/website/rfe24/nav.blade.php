<nav class="w-full z-20 py-3 top-0 start-0  text-white {{Route::is('home') ? 'bg-black sticky': 'bg-black sticky'}}"
    id="nav-home" data-color="#000000">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <a href="{{route('home')}}" class="flex items-center space-x-3 rtl:space-x-reverse">
            <img src="{{asset('assets/img/Logo_WHITE.png')}}" class="h-20" alt="ivao_logo">
        </a>
...
<script>
    $(window).on("scroll touchmove", function() {
        if ($(document).scrollTop() >= $("#eventDetails").position().top - 80) {
          ...
          $('#nav-home').css('background', '#000000');
